...
<div class="row">
    <div class="col-6 mb-5">
        <h4 class="mt-1 mb-1">States and Union Territories</h4>
        <p class="mb-0"><a href="{{route('dashboard')}}">Home</a> / States</p>
    </div>
    <div class="col-6 mb-5 text-end pt-5 pe-5">
        ...
        <a href="{{route('add_state')}}" class="btn btn-primary waves-effect waves-light"><i class="ri-add-line"></i> Add State</a>
    </div>
    <div class="col-12">
        <!-- States Table -->
        <div class="card">
        ...
        columns:[
            ...
            { "data": "name" ,"defaultContent": "-"},
            {
                ...
                "render": function (data, type, row, meta) {
                    ...
                    return type === 'display' ?
                    '<a href="{{asset("/")}}edit_state/' + row.id + '" class="btn btn-sm btn-icon btn-text-secondary rounded-pill delete-record waves-effect waves-light"><i class="ri-edit-box-line ri-20px"></i></a><button onClick="deleted('+row.id+')" class="btn btn-sm btn-icon btn-text-secondary rounded-pill delete-record waves-effect waves-light"><i class="ri-delete-bin-7-line ri-20px"></i></button><label class="switch"><input type="checkbox" class="switch-input" '+checked+' onclick="changeStatus('+row.id+')"/><span class="switch-toggle-slider"><span class="switch-on"></span><span class="switch-off"></span></span></label>' :
                    data;
                }
            }
            ...
            if(result.dismiss != 'cancel'){
                $.ajax({
                    ...
                    success:function(result)
                    {
                        Swal.fire({
                            title: 'Deleted!',
                            text: "The state deleted succsessfully!",
                            icon: 'success',
                            showCancelButton: false,
                            confirmButtonText: 'ok',
                            customClass: {
